...
def test_disponivel_desconta_vendidos_e_reservas_abertas():
    ...
    assert available_count(session, SeatCounts(tickets_sold=30, reserved_open=20)) == 50

def test_disponivel_nunca_e_negativo():
    session = _session(capacity=10)

    assert available_count(session, SeatCounts(tickets_sold=8, reserved_open=5)) == 0

def test_status_a_venda_quando_ha_lugar_e_a_sessao_e_futura():
    session = _session()

    assert session_status(session, 50, datetime.now(timezone.utc)) == "on_sale"

def test_status_esgotado_quando_nao_sobra_lugar():
    session = _session()

    assert session_status(session, 0, datetime.now(timezone.utc)) == "sold_out"
...
def test_status_encerrado_depois_do_inicio_da_sessao():
    ...
    assert session_status(session, 50, depois) == "closed"
...
def test_status_cancelado_vence_encerrado_e_esgotado():
    ...
    assert session_status(session, 0, depois) == "cancelled"
